...
export default function CsvImportModal({ isOpen, onClose, onSuccess }) {
  ...
  const downloadTemplate = () => {
    const csvContent = 'nombre,tipo,descripcion\n"Mantenimiento","salida","Gastos de mantenimiento general"\n"Servicios Públicos","salida","Agua, luz, gas, etc."\n"Ingresos por Ventas","entrada","Ingresos por ventas de productos"\n"Cuotas de Socios","entrada","Ingresos por cuotas mensuales"\n"Equipamiento Deportivo","salida","Compra de balones, uniformes, etc."';
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement('a');
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', 'plantilla_generales.csv');
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      for (const row of data) {
        try {
          // Validate required fields (support both English and Spanish headers)
          const name = row.name || row.nombre;
          const type = row.type || row.tipo;
          const description = row.description || row.descripcion || '';
          
          if (!name || !type) {
            errors.push(`Fila con nombre "${name || 'sin nombre'}": faltan campos requeridos`);
            errorCount++;
            continue;
          }
          ...
          if (type !== 'entrada' && type !== 'salida') {
            errors.push(`Fila "${name}": tipo debe ser "entrada" o "salida"`);
            errorCount++;
            continue;
          }
      ...
      if (successCount > 0) {
        setUploadSuccess(`Se importaron ${successCount} categorías generales exitosamente.`);
        onSuccess();
      }
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 flex-1">
                ...
                <p className="mt-1 text-sm text-blue-700">
                  Descarga la plantilla CSV con el formato correcto para importar categorías generales.
                </p>
                <button
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            ...
            <ul className="text-xs text-gray-600 space-y-1">
              <li>• <strong>nombre:</strong> Nombre de la categoría general (requerido)</li>
