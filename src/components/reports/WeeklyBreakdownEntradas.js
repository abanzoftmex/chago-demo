...
const WeeklyBreakdownEntradas = ({
  ...
  const { disabledRows, toggleRow } = usePersistedDisabledRows(user?.uid, "entradas");
  const { selectedWeekOverview, handleWeekOverviewClick, clearWeekOverview } = useWeekDayBreakdown({
    transactions, generals, concepts, subconcepts, filters, currentDate, type: "entrada", soloPagados,
  });
  ...
  const handleRowClick = (week, index, subconcept, amount) => {
    const weekTransactions = transactions.filter((t) => {
      ...
      return t.type === "entrada" && tTime >= week.startTimestamp && tTime <= week.endTimestamp && fullName === subconcept;
    });
    ...
    <div className="bg-background rounded-lg border border-border p-6">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold text-lime-700">
          Resumen Mes {currentMonthName} - {montosPagados ? "Pagos recibidos" : "Entradas"}
        </h3>
      </div>
      ...
      <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-border">
            <thead className="bg-lime-100">
              <tr>
                <th rowSpan={2} className="px-4 py-2 text-center text-xs font-bold text-lime-800 bg-lime-200 tracking-wider border-r-2 border-lime-200">
                  Concepto
                </th>
                <th colSpan={wb.weeks.length} className="px-4 py-2 text-center text-xs font-bold bg-lime-200 text-lime-800 tracking-wider border-r-2 border-lime-200">
                  Semanas
                </th>
                <th rowSpan={2} className="px-4 py-2 text-center text-xs font-bold tracking-wider text-lime-800 bg-lime-200">
                  Totales
                </th>
              </tr>
              <tr>
                {wb.weeks.map((week, index) => {
                  return (
                  <th key={index} className="px-4 py-3 text-center text-xs font-medium text-lime-800 tracking-wider bg-lime-50">
                    <button
            ...
            <tbody className="bg-background divide-y divide-border">
              {Object.entries(wb.entradas || {}).map(([subconcept, weekData]) => {
                ...
                <tr key={subconcept} className={`hover:bg-muted/50 transition-opacity ${!isActive ? 'opacity-40' : ''}`}>
                  <td className="px-4 py-3 text-xs text-foreground min-w-[200px] max-w-[230px]">
                    <div className="flex items-start gap-2">
                      <button
                        ...
                        className={`mt-0.5 relative inline-flex h-4 w-7 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${isActive ? 'bg-lime-500' : 'bg-gray-300'}`}
                      >
                    ...
                    <td key={index} className="px-4 py-3 whitespace-nowrap text-xs text-right">
                      {amount ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-lime-100 hover:bg-lime-200 text-lime-700 hover:text-lime-800 font-medium transition-colors cursor-pointer border border-lime-300"
                        >
                          <EyeIcon className="h-3 w-3" />
                  ...
                  <td className="px-4 py-3 whitespace-nowrap text-xs text-right font-bold text-lime-800 bg-lime-100">
              ...
              <tr className="bg-gray-200 font-bold border-t-2 border-gray-400">
                ...
                {wb.weeks.map((week, index) => {
                  const weekTotal = Object.entries(wb.entradas || {}).reduce(
                ...
                <td className="px-4 py-3 whitespace-nowrap text-xs text-right font-bold text-lime-800 bg-lime-200">
                  {formatCurrency(Object.entries(wb.entradas || {}).reduce(
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-white/90 text-sm mt-0.5">
                    {selectedWeekDetail.weekRange} • Entradas
                  </p>
                </div>
                <button
...
export default WeeklyBreakdownEntradas;
